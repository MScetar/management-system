...
* 代码生成： 前端配置即可生成前后台所需的crud文件，并同步到项目目录中（专业版）
* 表单构建： 可视化低代码表单设计器，后期规划自动生成前后台，并提供模板方便导入所需表单（专业版）
...
7. 启动项目（初始账号：superadmin 密码：123456）
    ...
    使用celery【计划任务】需要额外启动celery 和 beat（调度器）（精简版已去除定时任务）
    mac/linux:
