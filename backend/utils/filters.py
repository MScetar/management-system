...
class TrafficViolationTimeFilter(django_filters.rest_framework.FilterSet):
    """
    用户管理 简单过滤器
    URL格式：http://127.0.0.1:8000/?start_time=2020-12-02 12:00:00&end_time=2021-12-13 12:00:00
    field_name: 过滤字段名，一般应该对应模型中字段名
    ...
    """
    #开始时间
    beginAt = django_filters.DateTimeFilter(field_name='violation_time', lookup_expr='gte')  # 指定过滤的字段
    #结束时间
    endAt = django_filters.DateTimeFilter(field_name='violation_time', lookup_expr='lte')
    # 模糊搜索
    car_number = django_filters.CharFilter(field_name='car_number', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    # 模糊搜索
    violation_type = django_filters.CharFilter(field_name='violation_type', lookup_expr='icontains')  # icontains表示该字段模糊搜索
    # 模糊搜索
    has_paid = django_filters.CharFilter(field_name='has_paid')

    class Meta:
        model = TrafficViolation
        fields = ['beginAt', 'endAt','car_number','violation_type','has_paid']
